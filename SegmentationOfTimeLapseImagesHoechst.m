function  SegmentationOfTimeLapseImagesHoechst(A,B)
...
cd(finaldirname)
    ...
    cd ..
    disp(scenename)
    segmentationHoechst(FinalImage,subdirname,scenename,fname,'Hoechstbinary_flat');
...
function If = segmentationHoechst(FinalImage,subdirname,scenename,filename,channel)
fig=1;
mkdir(strcat('Hoechstbinary_flat'));
...
zerostrel = 6;
firststrel = round(60./(dimdiff.^2));
sigmafirst = firststrel.*5;
kernelgsizefirst = firststrel.*9;
...
[a,b] = regexp(filename,'(_mKate|Hoechst|EGFP|DIC)');
...
% cd('tiffs')
% % imwrite(uint8(Ie),strcat(scenename,'_','t',time,'_Hoechstbinary_flat.tif'));
