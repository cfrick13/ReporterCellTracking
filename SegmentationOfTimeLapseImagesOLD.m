...
cd(finaldirname)
    ...
    cd ..
    disp(scenename)
    segmentationCFP(FinalImage,subdirname,scenename,fname,'CFPbinary_flat');
...
function If = segmentationCFP(FinalImage,subdirname,scenename,filename,channel)
fig=1;
mkdir(strcat('CFPbinary_flat'));
...
%this image will go into the watershed

% Igo = imopen(Ih,strel('disk',round(12./dimdiff)));
Igo = imopen(Ih,strel('disk',round(16./dimdiff)));
Igcfopendil = Igo;
%this image will be the edge definer

% finalsigma=20;
% finalkernelgsize=40;
gaus = gaussianBlurz(IobrcbrF,round(sigmafirst),(kernelgsizefirst));
...
%NEW
se = strel('disk',1);
If = imerode(Igcfopendil,se);

%%%check segmentation quaility
% prim = bwperim(If);
% scimg = (double(imgorig)./max(max(double(imgorig)))).*254;
% sBKGbinaryimg = uint8(scimg);
% sBKGbinaryimg(prim) = 255;
% imagesc(sBKGbinaryimg)
...
% cd('tiffs')
% % imwrite(uint8(Ie),strcat(scenename,'_','t',time,'_CFPbinary_flat.tif'));
