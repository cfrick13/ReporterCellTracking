...
global framesForDir ImageDetails  psettings

framesThatMustBeTracked = psettings.framesThatMustBeTracked;
if ~isempty(framesThatMustBeTracked)
idx = framesThatMustBeTracked(2)
else
...
smooththat=0;
[Smad,Cfp,CfpFC,SmadFC,Smadbkg,Cfpbkg] = plotthemfunction(framesThatMustBeTracked,Tracked,A,ImageDetails,SceneDirectoryPath,timeFrames,framesForDir,PlotAxes,imgsize,plottingON,psettings,makeIMG,makeIMGidx,smooththat);
...
SecondPlotAxes.XLim = ([framesThatMustBeTracked(1)-4 framesThatMustBeTracked(2)+2]);
...
PlotAxes.XLim = ([framesThatMustBeTracked(1)-4 framesThatMustBeTracked(2)+2]);
...
ThirdPlotAxes.XLim = ([framesThatMustBeTracked(1)-4 framesThatMustBeTracked(2)+2]);
ThirdPlotAxes.YLim = ([0 6]);

end
function [Smad,Cfp,CfpFC,SmadFC,Smadbkg,Cfpbkg] = plotthemfunction(framesThatMustBeTracked,Tracked,A,ImageDetails,SceneDirectoryPath,timeFrames,framesForDir,PlotAxes,imgsize,plottingON,psettings,makeIMG,makeIMGidx,smooththat)
...
Cfp = cellfun(@median,cfppxls,'UniformOutput',1);
Cfp(Cfp==123456789) = NaN;



basalcfp = nanmean(Cfp(:,framesThatMustBeTracked(1)-4:framesThatMustBeTracked(1)),2);
...
basalsmad = nanmean(Smad(:,framesThatMustBeTracked(1)-4:framesThatMustBeTracked(1)),2);
...
%                     makeIMG(jy,:) = ~logical(cellfun(@(x) length(x)==1,PX,'UniformOutput',1)); %choose only the cells without NAN
                ...
                makeIMGidx = find(makeIMG==1);
                smooththat=0;
                [Smad,Cfp,CfpFC,SmadFC,Smadbkg,Cfpbkg] = plotthemfunction(framesThatMustBeTracked,Tracked,A,ImageDetails,SceneDirPath,timeFrames,framesForDir,PlotAxes,imgsize,plottingON,psettings,makeIMG,makeIMGidx,smooththat);
            end
