...
E={datename}; %beginning of file name for saving
 ...
 mdir = mfilename('fullpath');
    ...
    expPath = strcat(parentdir,datename);
cd(expPath)
...
mstackPath = strcat(parentdir,datename,'/',mstackdir);


metadir = strcat(parentdir,'TrackingMstack/Export');
cd(metadir)
...
%         bki = bki;
    ...
    cfile = {filelist.name};
        ...
        parfor j = 1:length(cfile) %load image_stack for each scene // and determine scene and channel
            ...
            SAVdir = strcat(parentdir,datename,'/',flatfielddirname,'/');
